...
for i in {1..30}; do
    # Usamos curl para comprobar si la URL responde con un código 2xx (éxito)
    # El output se descarta (> /dev/null)
    if curl -s -f http://localhost:8080/Laboratorio > /dev/null; then
        echo "==> ¡DESPLIEGUE COMPLETADO Y EXITOSO! La aplicación esta lista."
        # Trae el proceso de WildFly al primer plano. El contenedor se mantendrá vivo.
        wait $WILDFLY_PID
        exit $?
    fi
